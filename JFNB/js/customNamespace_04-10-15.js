...
ADF = {
    ...
    Methods: {
        ...
        submitDonation: function() {
            ...
            //turn these on only if not using PP2.0
                if ($('#processDirectly').length !==0) {
                    donation.Gift.MerchantAccountId = ADF.Settings.merchantID;
                    donation.Gift.BBSPReturnUri = window.location.href;
                }
        ...
        convertErrorToString: function(error) {
            if (error) {
                var messageToReturn = '';
                if (error.ErrorCode) {
                    messageToReturn = messageToReturn + error.ErrorCode + ": ";
                }
                if (error.Field) {
                    messageToReturn = messageToReturn + error.Field + "- ";
                }
                if (error.Message) {
                    messageToReturn = messageToReturn + error.Message + ". ";
                }

                switch (error.ErrorCode) {
                    case 101:
                        messageToReturn = messageToReturn + error.Field + " is required.";
                    case 105:
                        messageToReturn = messageToReturn +  error.Field + " is not valid.";
                    case 106:
                        messageToReturn = messageToReturn +  "Record for " + error.Field + " was not found.";
                    case 203:
                        messageToReturn = messageToReturn +  "Donation not completed on BBSP.";
                    case 107:
                        messageToReturn = messageToReturn +  "Max length for " + error.Field + " exceeded.";
                    default:
                        messageToReturn = messageToReturn +  "Error code " + error.ErrorCode + ".";
                }
