...
ADF = {
    Settings: {
        ...
        merchantID      : '46b94b1e-383b-479f-8f7e-bba4ea285c98',    //'b3d5af59-9f26-4414-9f9a-b13f7aae7899',
    ...
    Methods: {
        initializeADF: function() {
            ...
            if (transactionID) {
              ...
              completeBBSPPayment(transactionID);
                ...
                getEvents();
                //get states from CRM
                ADF.Methods.bindStateDropdown($('#adfGiftState, #adfBillingState'));
                ...
                $('.adfSubmitWrapper input[type="button"]').on('click', function() {
                    $('.adfTextInput, .adfSelectInput, #adfBillingAddress, #adfGiftAddress').each(function() {
                   ...
                   if ($('input[name="giftCardDesign"]:checked').length === 0) {
                    ...
                    $('#adfBillingEmail, #adfGiftEmail').each(function() {
                        if ($(this).val().match('@') == -1) {
                            setInvalid($(this), 'Invalid email address');
                        }
                    });
                    $('.adfTextInput.invalid, #adfBillingAddress, #adfGiftAddress').unbind('focus').on('focus', function() {
                        removeInvalid($(this));
                    });
        ...
        submitDonation: function() {
            ...
            var donation = {
                "Donor": {
                    "Address": {
                        "City": $('#adfBillingCity').val(),
                        "Country": ADF.Settings.countryID,
                        "PostalCode": $('#adfBillingZip').val(),
                        "State": $('#adfBillingState').val(),
                        "StreetAddress": $('#adfBillingAddress').val()
                    },
                    "EmailAddress": $('#adfBillingEmail').val(),
                    "Phone" : $('#adfBillingPhone').val(),
                    "FirstName": $('#adfBillingFirstName').val(),
                    "LastName": $('#adfBillingLastName').val()
                },
                "Gift" : {
                    "Designations": [{
